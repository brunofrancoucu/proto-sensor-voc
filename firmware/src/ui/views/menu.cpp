...
void MenuView::paint() {
    ...
    int16_t x1, y1;
    uint16_t w, h;
    oled.getTextBounds(labels[focusedId], 0, 0, &x1, &y1, &w, &h);
    // Draw
    const uint8_t gap = 8;
    const uint8_t pdg = 8;
    ...
    oled.drawBitmap((SSD_WIDTH - rectWidth)/2 + pdg, 23, icons[focusedId], 16, 16, 1);
    oled.setCursor((SSD_WIDTH - rectWidth)/2 + 16 + gap + pdg, 27); // 64/2 - 8/2
    oled.print(labels[focusedId]);
    
    // Icons Left
    for (size_t i = 0; i < labels.size(); ++i) {
        int posX = SSD_WIDTH / 2 - rectWidth / 2 - i * (16 + 10) - 8 - 16;
        oled.drawBitmap(posX, 23, icons[(focusedId - i - 1 + labels.size()) % labels.size()], 16, 16, 1);
    }
    
    // Icons Right
    for (size_t i = 0; i < labels.size(); ++i) {
        int posX = SSD_WIDTH / 2 + rectWidth / 2 + i * (16 + 10) + 8;
        oled.drawBitmap(posX, 23, icons[(focusedId + i + 1) % labels.size()], 16, 16, 1);
    }
}
